...
def init_db():
    ...
    if table_exists:
        ...
        # Check if we need to add recurrence columns
...
def migrate_to_recurring_events(conn):
    """Add recurrence columns to events table."""
    cursor = conn.cursor()
    
    # Add recurrence_id column
    cursor.execute('ALTER TABLE events ADD COLUMN recurrence_id TEXT')
    
    # Add rrule column
    cursor.execute('ALTER TABLE events ADD COLUMN rrule TEXT')
    
    # Add original_start column
    cursor.execute('ALTER TABLE events ADD COLUMN original_start TEXT')
    
    conn.commit()
    print("Added recurring events columns to events table")
...
def add_event(start_datetime, end_datetime, title, description='', tag='', user_id=None, rrule=None, recurrence_id=None):
    ...
    # Set original_start for recurring events
    original_start = start_datetime if rrule else None
